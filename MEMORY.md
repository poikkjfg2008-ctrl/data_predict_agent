...
- 用户新增方向：希望在当前 agent 项目中接入 Ollama 本地推理接口，并希望交付 skill 化（`SKILL.md`）和可直接填空的配置模板。
- 本次产出：新增 `skills/ollama-agent-integration/SKILL.md`，以及 `configs/ollama/` 下 `.env`、`yaml`、`docker-compose` 示例。
